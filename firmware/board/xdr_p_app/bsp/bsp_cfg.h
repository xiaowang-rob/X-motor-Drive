...
// 装配并启动全板设备：存储 → IAP → 功率级 → 采样 → 灯 → 通讯。
// 返回 true 表示关键设备（存储 + 功率级 + 采样）装配成功。
bool dev_base_init(void);

// 板载编码器启动（型号固定，ops/handle 已在定义处挂好）
bool dev_int_enc_init(eEncoderMode mode);

// 外接编码器启动：型号来自参数，按 chip_code 选中驱动后挂钩再启动。
// chip_code：0 未装配 / 1 MT6816 / 2 MT6835 / 3 AS5047
